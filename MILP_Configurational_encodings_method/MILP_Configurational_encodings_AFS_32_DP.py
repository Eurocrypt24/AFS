...
def gubi(file, round_num, rot, k, s, Seq):          ###########Gurobi Optimizer
    ...
    point1 = "./" + "32bit_" +str(s)+ "_diff_" + str(round(round_num/2)) + "Round_"+str(k)+".txt"
...
def differ(blocksize2, k1, round_num, rot, s1, Seq):
    ...
    filename = "./" + "32bit_" + str(s1)+ "_diff_" + str(round(round_num/2))+ "Round_"+str(k1)+".lp"
    o = open(filename, 'w')
    ...
    for r in range(round_num):
        if s1[(r%8)] == 1:
            rotr(X[r][1], k1[(r%8)])
            o.write(xor_Constraint(X[r][0][15], X[r][1][15], X[r+1][1][15]))
...
def main():
    blocksize2 = 16 ###S-box branch size
...
##############AFS-32(11000011)##################
    s = [[1,1,0,0,0,0,1,1]]
    k = [[9,8,2,1,7,15,14,0],[0,1,9,15,14,8,2,7]]

##############AFS-32(10100011)##################
    # s = [[1,0,1,0,0,0,1,1]]
    # k = [[0,7,14,15,8,1,2,9],[8,0,2,1,9,14,7,15]]
    ...
    for Seq in range(0,len(s)):
        for rot in range(0, len(k)):
            ...
            ####################################Output Result File Storage Path
            point = "./"+"32bit_"+ str(s[Seq])+ "_diff_"+ str(round(round_num/2)) + "Round_" + str(k[rot])+ ".txt"
