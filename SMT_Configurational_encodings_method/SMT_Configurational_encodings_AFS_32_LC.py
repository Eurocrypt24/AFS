...
def AFS_P(round, cvc_path1, prob, s, k, s_l, k_l):
    ...
    for r in range(ROUNDS+1):
        ...
        fp.write("x_{}: BITVECTOR(16) ;".format(r))  # Left input linear characteristic for the r-th round: x_r;
        fp.write("\n")
       
        fp.write("y_{}: BITVECTOR(16) ;".format(r))  # Right input linear characteristic for the r-th round: y_r;
        fp.write("\n")
    ...
    for r in range(ROUNDS):
        if s[r%s_l] == 1:
            fp.write("p_{}: BITVECTOR(16) ;".format(r))  # Correlation auxiliary variable for modular addition;
            fp.write("\n")

            fp.write("z_{}: BITVECTOR(16) ;".format(r))  # Linear characteristic before the rotation operation in the r-th round: z_r
            fp.write("\n")
  
    fp.write("total_probability: BITVECTOR(16) ;")  # Total target Correlation;
    fp.write("\n")
    fp.write("ASSERT( NOT( (x_0 @ y_0) = 0hex00000000) );\n") # Input Linear characteristic are not all zero;
    ...
    for r in range(ROUNDS):
        ...
        if s[r%s_l] == 1:
            if k[r%k_l] != 0:
                ...
                fp.write("ASSERT( p_" + str(r) + " = (0bin0)@(BVXOR(BVXOR( x_{},(z_{}[{}:0])@(z_{}[15:{}]) ), y_{})[{}:{}])".format(r, r, k[r % k_l]-1, r, k[r % k_l], r+1, 15, 15))
                for i in range(2,16): #1~n-1,   n=16
                    fp.write("@(")
                    for j in range(2, i+1):
                        fp.write("BVXOR(")
                    fp.write("BVXOR(BVXOR( x_{},(z_{}[{}:0])@(z_{}[15:{}]) ), y_{})[{}:{}]".format(r,r,k[r % k_l]-1,r,k[r % k_l],r+1,15,15))
                    for j in range(2, i + 1):
                        fp.write(",BVXOR(BVXOR( x_{},(z_{}[{}:0])@(z_{}[15:{}]) ), y_{})[{}:{}] )".format(r,r,k[r % k_l]-1,r,k[r % k_l],r+1,16-j,16-j))
                    fp.write(")")
                fp.write(") ;\n")
                ##############################
                ...
                fp.write("ASSERT( BVXOR( y_{},(z_{}[{}:0])@(z_{}[15:{}]) ) = ( BVXOR( y_{},(z_{}[{}:0])@(z_{}[15:{}]) )&p_{}) ) ;\n".format(r + 1, r,k[r % k_l]-1,r,k[r % k_l],r+1,r,k[r % k_l]-1,r,k[r % k_l],r) )  ### Condition for the effective linear feature propagation in modular arithmetic operations
            ...
            if k[r%k_l] == 0:
                ...
                ################################ p_r = (M_n)^T(y_r+1,x_r,z_r)
                fp.write("ASSERT( p_" + str(r) + " = (0bin0)@(BVXOR(BVXOR( x_{}, z_{} ), y_{})[{}:{}])".format(r, r, r+1, 15, 15))
                for i in range(2,16): #1~n-1,   n=16
                    fp.write("@(")
                    for j in range(2, i+1):
                        fp.write("BVXOR(")
                    fp.write("BVXOR(BVXOR( x_{}, z_{} ), y_{})[{}:{}]".format(r,r,r+1,15,15))
                    for j in range(2, i + 1):
                        fp.write(",BVXOR(BVXOR( x_{}, z_{} ), y_{})[{}:{}] )".format(r,r,r+1,16-j,16-j))
                    fp.write(")")
                fp.write(") ;\n")
                ##############################
        ...
        if s[r%s_l] == 0:
            ...
            if k[r%k_l] != 0:
                ...
                fp.write("ASSERT( x_{} = BVXOR(y_{}, (x_{}[{}:0])@(x_{}[15:{}]) ) ) ;\n".format(r+1, r, r, 15-k[r % k_l], r, 16-k[r % k_l]))  ##linear propagation of branch operations: x_r+1 = y_r xor (x_r<<k[r])
    ...
    for r in range(ROUNDS):
        if s[r%s_l] == 1:
            for i in range(0, 15): ####Addition of correlation-assisting bits from the (n-2)th position to the least significant bit at position 0.
...
def main():
    ...
    for s_i in range(len(s_pool)):
        for k_i in range(len(k_pool)):
            ...
            PATHTEMP1 = "./" + "32bit_LC_s_" + str(s) + "k_" + str(k) + "r_" + str(rd) + "low_" + str(low) + ".cvc"     ###Output/Input SMT Model CVC File Storage Path
            PATHTEMP2 = "./" + "32bit_LC_s=" + str(s) + ";k=" + str(k) + ";r=" + str(rd) + ";low=" + str(low) + ".txt"  ###Output Result File Storage Path
